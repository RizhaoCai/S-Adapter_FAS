...
class Trainer():
    ...
    def contrastive_loss_inner_product_(self, prototype_1, prototype_2, feature, spoofing_label, temperature=10, with_negative=True):

        marginal = 1
        # category 0
        # import pdb; pdb.set_trace()
        exp_item1 = torch.exp(torch.inner(feature, prototype_1) / temperature)
        exp_item2 = torch.exp(torch.inner(feature, prototype_2) / temperature)
        metric1 = -torch.log(exp_item1 / (exp_item1 + exp_item2))
        # -metric1*spoofing_label (clusttering the real face)  ((1-spoofing_label)*distance_1)  -> pull away fake from the real cluster

        contrastive_real = ((1-spoofing_label)*metric1).mean()
        contrastive_fake = max(torch.tensor(0.0), (marginal-spoofing_label * metric1).mean())

        # import pdb; pdb.set_trace()
        return contrastive_real, contrastive_fake

    def contrastive_loss_inner_product(self, prototype_1, prototype_2, feature, spoofing_label, temperature=10):
