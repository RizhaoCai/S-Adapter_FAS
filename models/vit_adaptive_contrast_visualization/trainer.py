...
class Trainer():
    ...
    def __init__(self, config):
        ...
        kwargs = {
            ...
            'adaptive_type': self.config.MODEL.ADAPTIVE_TYPE,
        }
    ...
    def train(self, ):
        ...
        hook_handle = self.network.module.head.register_forward_hook(hook_feature)
        self.contrast_loss_func = SupConLoss()
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            with tqdm(total=num_train, ncols=80) as pbar:
                for i, batch_data in enumerate(train_data_loader):
                    ...
                    contrastive_loss = self.contrast_loss_func(feature_norm.unsqueeze(1), spoofing_label)
    ...
    def test(self, test_data_loader):
        ...
        with torch.no_grad():
            ...
            for data in tqdm(test_data_loader, ncols=80):
                ...
                image_index = 0
                layer_index = 11
                image_path = video_ids[image_index]

                # Detransform

                cv2_images = data[4]
                img = Image.fromarray(cv2_images[image_index].numpy())

                #import pdb; pdb.set_trace()
                for patch_index in range(1,195,7):
                    visualize_grid_to_grid(attention_maps[layer_index][image_index,0,1:,1:],patch_index , img, '{}/layer_{}_image_{}_patch_{}.png'.format(self.config.OUTPUT_DIR, layer_index, counter, patch_index))
                counter += 1
