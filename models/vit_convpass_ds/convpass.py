...
class Convpass(nn.Module):
    def __init__(self, dim=8, xavier_init=False, ConvClass=Conv2d_cd):
        super().__init__()

        self.adapter_conv = ConvClass(dim, dim, 3, 1, 1)
        if xavier_init:
...
class DS_Convpass(nn.Module):
    def __init__(self, stream1, stream2):
        super().__init__()
        self.stream1 = stream1
        self.stream2 = stream2
    def forward(self, x):
        return (self.stream1(x) + self.stream2(x))/2
    ...
    if method == 'convpass':
        ...
        for _ in model.children():
            if type(_) == timm.models.vision_transformer.Block:
                _.adapter_attn = DS_Convpass(
                    stream1=Convpass(dim, xavier_init, ConvClass=Conv2d_Hori_Veri_Cross),
                    stream2=Convpass(dim, xavier_init, ConvClass=Conv2d_Diag_Cross)
                )

                _.adapter_mlp = DS_Convpass(
                    stream1=Convpass(dim, xavier_init, ConvClass=Conv2d_Hori_Veri_Cross),
                    stream2=Convpass(dim, xavier_init, ConvClass=Conv2d_Diag_Cross)
                )
                _.s = s
        ...
        for _ in model.children():
            if type(_) == timm.models.vision_transformer.Block:
                _.adapter_attn = Convpass(dim, xavier_init, ConvClass=conv_type)
                _.s = s
