...
import models.vit_adaptive_contrast_visualization.timm_vit as timm_vit
import logging
...
class Conv2d_cd(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 padding=1, dilation=1, groups=1, bias=False, adaptive_type='sample'):
    ...
    def forward(self, x):
        ...
        if math.fabs(theta - 0.0) < 1e-8:
            ...
            if self.adaptive_type == 'learnable' or self.adaptive_type == 'sample':
                # theta = (1+torch.tanh(self.theta)) # constrain to 0~1
...
def forward_block(self, x):
    attention_map = self.adapter_attn(self.norm1(x))
    x = x + self.drop_path1(self.attn(self.norm1(x))) + self.drop_path1(attention_map) * self.s
...
class Convpass(nn.Module):
    ...
    def forward(self, x):
        ...
        x_down = self.act(x_down) # 64， 197, 8

        x_patch = x_down[:, 1:].reshape(B, 14, 14, self.dim).permute(0, 3, 1, 2) # 64,8, 14,14
        x_patch = self.adapter_conv(x_patch) # 64,8, 14,14
        x_patch = x_patch.permute(0, 2, 3, 1).reshape(B, 14 * 14, self.dim) # 64,196, 8
