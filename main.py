...
import os, sys
import time
from utils.utils import save_checkpoint, adjust_learning_rate, AverageMeter, accuracy, load_model_pytorch, \
    dynamic_network_change_local, get_conv_sizes, connect_gates_with_parameters_for_flops
...
def main():
    ...
    parser.add_argument('--dataset', default="MNIST", type=str,
                        ...
                        choices=["MNIST", "CIFAR10", "Imagenet", 'pacs', 'digit', 'office'])
    ...
    parser.add_argument('--model', default="lenet3", type=str,
                        ...
                        choices=["lenet3", "vgg", "mobilenetv2", "resnet18", "resnet18_noskip", "resnet152", "resnet50",
                                 "resnet50_noskip",
    ...
    parser.add_argument('--fix_bn', default=False, type=str2bool, nargs='?',
                        help='Fix BN layer during training')
    ...
    parser.add_argument('--reset_criteria', default=False, type=str2bool, nargs='?',
                        help='If True reset the importance score after pruning')
    ...
    elif args.dataset in ['pacs', 'digit', 'office']:
        from riz.dataset import get_pacs_dataset
        from riz.dataset import get_digit_dataset
        from riz.dataset import get_office_dataset

        protocol = args.protocol
        # if _to_photo
        if_digit = args.dataset == 'digit'
        if 'resnet18' in args.model:
            skip_gate = True  # With Gate Layer
            if "noskip" in args.model:
                skip_gate = False  # Without Gate Layer

            if args.pruning_method not in [22, 40]:
                # Pruning methods of 22 and method 40 should use Gate Layer
                skip_gate = False  # Without Gate Layer

            model = resnet18(num_classes=args.num_classes, skip_gate=skip_gate, if_digit=if_digit)
            ...
            if args.pruning_method not in [22, 40]:
                # Pruning methods of 22 and method 40 should use Gate Layer
                skip_gate = False  # Without Gate Layer
            model = resnet50(num_classes=args.num_classes, skip_gate=skip_gate, if_digit=if_digit)


        elif 'vgg' in args.model and args.dataset == 'pacs':
            model = vgg11_bn(pretrained=False, num_classes=7)

        if args.dataset == 'pacs':
            train_dataset_name = 'pacs_' + protocol + '_train'
            ...
            train_dataset = get_pacs_dataset(protocol_name=train_dataset_name, transform=transform)

            # datasets = get_pacs_dataset_split(protocol_name=dataset_name_, transform=transform)
            val_dataset_name = 'pacs_' + protocol + '_val'
            ...
            transform = transforms.Compose([
                ...
                transforms.Normalize(
                    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
            val_dataset = get_pacs_dataset(protocol_name=val_dataset_name, transform=transform)
            test_dataset_name = 'pacs_' + protocol + '_test'
            test_dataset = get_pacs_dataset(protocol_name=test_dataset_name, transform=transform)

        elif args.dataset == 'office':
            train_dataset_name = 'office_' + protocol + '_train'
            ...
            transform = transforms.Compose([
                ...
                transforms.Normalize(
                    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
            train_dataset = get_office_dataset(protocol_name=train_dataset_name, transform=transform)

            # datasets = get_pacs_dataset_split(protocol_name=dataset_name_, transform=transform)
            val_dataset_name = 'office_' + protocol + '_val'
            ...
            transform = transforms.Compose([
                ...
                transforms.Normalize(
                    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
            val_dataset = get_office_dataset(protocol_name=val_dataset_name, transform=transform)
            test_dataset_name = 'office_' + protocol + '_test'
            test_dataset = get_office_dataset(protocol_name=test_dataset_name, transform=transform)
        elif args.dataset == 'digit':
            train_dataset_name = 'dg_' + protocol + '_train'
            transform = transforms.Compose([
                ...
                transforms.Normalize(
                    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
            train_dataset = get_digit_dataset(protocol_name=train_dataset_name, transform=transform)

            # datasets = get_pacs_dataset_split(protocol_name=dataset_name_, transform=transform)
            val_dataset_name = 'dg_' + protocol + '_val'
            ...
            transform = transforms.Compose([
                ...
                transforms.Normalize(
                    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
            val_dataset = get_digit_dataset(protocol_name=val_dataset_name, transform=transform)
            test_dataset_name = 'dg_' + protocol + '_test'
            test_dataset = get_digit_dataset(protocol_name=test_dataset_name, transform=transform)
    ...
    print("model is set to device: use_cuda {}, args.mgpu {}, agrs.distributed {}".format(use_cuda, args.mgpu,
                                                                                          args.distributed))
    ...
    # optimizer = optim.Adam(parameters_for_update, lr=args.lr, weight_decay=weight_decay) # Debug
    ...
    model.cuda()
    # Do testing first to check the accuracy before pruning
    if args.dataset in ['pacs', 'digit', 'office']:
        prec1_val, _ = validate(args, val_loader, model, device, criterion, -1, train_writer=train_writer)
        print('Initial Validation acc', prec1_val)

        prec1, _ = validate(args, test_loader, model, device, criterion, -1, train_writer=train_writer)
        print('Initial Test acc', prec1)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if not args.run_test and not args.get_inference_time:
            train(args, model, device, train_loader, optimizer, epoch, criterion, train_writer=train_writer,
        ...
        if args.dataset in ['pacs', 'digit', 'office']:
            prec1_val, _ = validate(args, val_loader, model, device, criterion, epoch, train_writer=train_writer)
            print('Validation acc', prec1_val)

        prec1, _ = validate(args, test_loader, model, device, criterion, epoch, train_writer=train_writer)
        print('Test acc', prec1)
