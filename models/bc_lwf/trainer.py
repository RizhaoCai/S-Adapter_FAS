...
def MultiClassCrossEntropy(logits, labels, T):
    # Logit: curent output, labels: output
...
class Trainer():
    ...
    def train(self):
        ...
        for task_id in range(1, num_of_tasks+1):
            ...
            teacher_network = copy.deepcopy(self.network)
            self.train_a_task( self.network, teacher_network, train_data_loader, val_data_loader, task_id, task_name)
    ...
    def train_a_task(self, network, teacher_network, train_data_loader, val_data_loader, task_id, task_name):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            with tqdm(total=num_train, ncols=80) as pbar:
                for i, batch_data in enumerate(train_data_loader):
                    ...
                    network_input, target = batch_data[1].cuda(), batch_data[2]

                    cls_out = self.inference(network_input)
                    ...
                    with torch.no_grad():
                        teacher_output = teacher_network(network_input)
                    distillation_loss = MultiClassCrossEntropy(cls_out, teacher_output, T=1.0)

                    #import pdb; pdb.set_trace()
                    loss = ce_loss + 0.1*distillation_loss

                    pbar.set_description(
                        (
                            " loss={:.4f}|ce_loss={:.4f}|distillation_loss={:.4f} ".format(loss.item(), ce_loss.item(), distillation_loss.item(),
                                                         )
                        )
                    )
