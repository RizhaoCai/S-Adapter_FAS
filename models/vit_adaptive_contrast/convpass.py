...
class Conv2d_cd(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
        ...
        if self.adaptive_type == 'learnable':
            ...
            self.mu = torch.nn.parameter.Parameter(torch.tensor(0.0, requires_grad=True))
            self.sigma = torch.nn.parameter.Parameter(torch.tensor(1.0, requires_grad=True))
    ...
    def reparameterization_sampling2(self, mu, sigma):
    ...
    def forward(self, x):
        ...
        if self.adaptive_type == 'learnable':
            ...
            Theta = self.reparameterization_sampling2(self.mu, self.sigma)
